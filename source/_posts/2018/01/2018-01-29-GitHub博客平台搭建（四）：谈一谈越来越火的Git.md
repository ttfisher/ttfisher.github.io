---
title: GitHub博客平台搭建（四）：谈一谈越来越火的Git（未完成...)
categories:
  - 多元化技能储备之Git&GitHub
tags:
  - GitHub
...
## create
&emsp;&emsp;首先，在自己的GitHub创建一个Repository（比如我创建了一个D02-GitPractice）；然后在本地磁盘创建一个临时的练习目录（比如我创建的D:\00-GitHub.Practice）。然后接下来就可以开始我们的Git实践之旅了。

## clone
&emsp;&emsp;要完成clone操作，首先要在GitHub仓库找到Repository的clone链接地址（通过【Clone or download】可以获取）；然后需要在本地的练习目录下，打开Git Bash窗口（右键即可找到对应的菜单项）；随后通过如下的clone命令完成仓库的clone。
```bash
Administrator@Tang94 MINGW64 /d/00-GitHub.Practice
...
## init
&emsp;&emsp;当然，实际使用时未必有GitHub环境，此时init就可以充分发挥它的价值了，在任意目录下通过如下命令操作即可创建一个初始化好的本地仓库；这么做和从中心仓下载的区别，个人觉得最核心的是中心仓可以供团队作战进行资源汇总和交互。
```bash
Administrator@Tang94 MINGW64 /d
$ git init 01-MyRepo
Initialized empty Git repository in D:/01-MyRepo/.git/
