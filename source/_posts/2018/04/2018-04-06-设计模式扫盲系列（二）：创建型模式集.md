...
## 定义
&emsp;&emsp;工厂模式（Factory Pattern）是 Java 中最常用的设计模式之一，它也有不同的变种（比如：简单工厂、抽象工厂）。用工厂方法代替了传统的new操作。在工厂模式中，我们在创建对象时不会对客户端暴露创建逻辑，并且是通过使用一个共同的接口来指向新创建的对象。
&emsp;&emsp;通俗的理解，工厂模式可以粗粒度的分两种：
- 第一种是简单工厂模式，顾名思义简单，我们只有一个工厂，但是有多条生产线，不同的生产线生产不同的具体产品，你只需要告诉工厂你想生产那种产品（输入参数）就可以了。
- 第二种就是常规的工厂模式，更具通用性，就是我们有多个工厂，每个工厂都可以生产，你需要哪种类型的产品，就直接找到对应类型的工厂就可以了。

## 类图
<div align=center><img src="https://github.com/ttfisher/images/raw/master/2018/2018-08-20-01.jpg" width="55%"/></div>

&emsp;&emsp;通过上面的类图我们可以清晰的分析出工厂模式的几个角色：
- Product：抽象的产品接口
- ConcreteProduct：具体的产品类
- Creator：抽象的工厂接口
- ConcreteCreator：具体的工厂接口实现类

&emsp;&emsp;针对以上四个角色（有时候会有一些简化，比如取消了抽象产品），实际给客户端使用时，对外则只需要暴露Creator这个接口就可以（实际的创建逻辑则不需要暴露），通过这个工厂接口即可完成Product也就是产品的创建过程。
...
### 工厂接口
&emsp;&emsp;重要的事情说三遍：我们是面向接口编程的，当然这里的工厂接口你要真的不想带，也是可以的，但是一旦你抛弃了接口若想扩展接口的实现时就会比较头疼了；这里使用一个工厂接口定义了生产产品的方法，当然工厂可能还有些其他的方法（也就是anOperation了，这个就和我们的设计模式没什么关系了）：
```java
/**
 * 工厂接口
 *
...
&emsp;&emsp;实际上看呢，简单工厂模式也是工厂模式的一种最简实现，它把抽象工厂的多个实现直接使用传入参数的模式模拟了，这种方法有一个致命的缺陷就是一旦需要新增工厂，那么就涉及到对工厂实现类的内部逻辑进行修改（这就违反了对修改关闭对扩展开放的原则），而实际的工厂模式由于使用的是对工厂接口的多个不同实现，在扩展时只需要增加实现就可以解决问题。不过话说回来简单工厂模式在某些场景下还是有用武之地的。
&emsp;&emsp;下面就直接用代码演示了一下，简单工厂模式之所以加了简单的描述语，是因为它把工厂接口的多实现省掉了，直接通过一个参数来判断需要生产什么产品（对于产品变化几率很小的情况下还是实用性很强的）。
```java
/**
 * 具体的工厂(简单工厂模式）
 *
...
&emsp;&emsp;通俗的理解，抽象工厂模式里有一个产品族的概念，每个工厂生产的多个产品之间有一定的联系，不同工厂间有可能有无法兼容的情况，为了避免让客户去梳理兼容逻辑，就在工厂端做了这种兼容性的限定。
...
## 类图
&emsp;&emsp;原型模式分三个角色，抽象原型类，具体原型类，客户类
