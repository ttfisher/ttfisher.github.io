...
<div align=center><img src="http://pm4hdun71.bkt.clouddn.com/img/2018/2018-05-24-07.jpg" width="500"/></div>
<!-- more -->
...
# log4j在工程中怎么引用？
&emsp;&emsp;如今Maven的流行，让我们很简单的就可以引用我们想要的各自软件包，不像多年前还需要在本地打个lib目录再到工程配置中去依赖，时代真的是在不断进步的。
```
<dependency>
...
# log4j的使用示例
&emsp;&emsp;其实我们日常使用的话，关注的无非就是以下几个点：
- 日志等级打印到什么级别？
- 日志输出到什么地方？
- 日志文件怎么命名的？
- 日志多了会不会把系统撑爆？

&emsp;&emsp;参考下面的例子，基本对log4j的基础使用包括上面提到的问题有个初步的概念了：
```bash
# 某舆情系统默认的Log4j配置文件
...
# 实例解析
&emsp;&emsp;前面已经有过一个实例了，实际上这里的实例写的也并不完全，但是从大体结构上对该有的配置也都包含到了，实际运用时可以参考前面的具体细则自行定制。
```
#全局日志等级配置,输出位置
